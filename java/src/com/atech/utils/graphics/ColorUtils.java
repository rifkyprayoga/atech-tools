package com.atech.utils.graphics;

/**
 * Created by andy on 23.01.16.
 */
...
public class ColorUtils
{
    ...
    private final static float U_OFF = .436f, V_OFF = .615f;
    ...
    private static float[] randYUVinRGBRange(float minComponent, float maxComponent)
    {
        while (true)
        {
            ...
            if (0 <= r && r <= 1 && 0 <= g && g <= 1 && 0 <= b && b <= 1
                    && (r > minComponent || g > minComponent || b > minComponent) && // don't
                                                                                     // want
                                                                                     // all
                                                                                     // dark
                                                                                     // components
            (r < maxComponent || g < maxComponent || b < maxComponent)) // don't
                                                                        // want
                                                                        // all
                                                                        // light
                                                                        // components
    ...
    /*
     * Returns an array of ncolors RGB triplets such that each is as unique from
     * the rest as possible
     * and each color has at least one component greater than minComponent and
     * one less than maxComponent.
     * Use min == 1 and max == 0 to include the full RGB color range.
     * Warning: O N^2 algorithm blows up fast for more than 100 colors.
     */
    public static Color[] generateVisuallyDistinctColors(int ncolors, float minComponent, float maxComponent)
    {
        rand.setSeed(RAND_SEED); // So that we get consistent results for each
        ...
        // continually break up the worst-fit color pair until we get tired of
        // searching
        ...
        for (int i = 0; i < yuv.length; i++)
        {
            ...
            // System.out.println(rgb[i][0] + "\t" + rgb[i][1] + "\t" +
            // rgb[i][2]);
        }
    ...
    private static float[] randYUVBetterThan(float bestDistSqrd, float minComponent, float maxComponent, float[][] in)
    {
        ...
        return null; // after a bunch of passes, couldn't find a candidate that
                     // beat the best.
    }
