...
public abstract class CRCUtils // extends BitUtils // was HexUtils
{
    ...
    private static int CRC8_LOOKUP_TABLE_BD[] = { 0, 94, 188, 226, 97, 63, 221, 131, 194, 156, 126, 32, 163, 253, 31,
    ...
    private static int CRC16_CCITT_LOOKUP_TABLE[] = { 0, 4129, 8258, 12387, 16516, 20645, 24774, 28903, 33032, 37161,
    ...
    private static int CRC7_LOOKUP_TABLE[] = { 0, 26, 52, 46, 104, 114, 92, 70, 93, 71, 105, 115, 53, 47, 1, 27, 55, 45,
                                               3, 25, 95, 69, 107, 113, 106, 112, 94, 68, 2, 24, 54, 44, 110, 116, 90,
                                               64, 6, 28, 50, 40, 51, 41, 7, 29, 91, 65, 111, 117, 89, 67, 109, 119, 49,
                                               43, 5, 31, 4, 30, 48, 42, 108, 118, 88, 66, 81, 75, 101, 127, 57, 35, 13,
                                               23, 12, 22, 56, 34, 100, 126, 80, 74, 102, 124, 82, 72, 14, 20, 58, 32,
                                               59, 33, 15, 21, 83, 73, 103, 125, 63, 37, 11, 17, 87, 77, 99, 121, 98,
                                               120, 86, 76, 10, 16, 62, 36, 8, 18, 60, 38, 96, 122, 84, 78, 85, 79, 97,
                                               123, 61, 39, 9, 19, 47, 53, 27, 1, 71, 93, 115, 105, 114, 104, 70, 92,
                                               26, 0, 46, 52, 24, 2, 44, 54, 112, 106, 68, 94, 69, 95, 113, 107, 45, 55,
                                               25, 3, 65, 91, 117, 111, 41, 51, 29, 7, 28, 6, 40, 50, 116, 110, 64, 90,
                                               118, 108, 66, 88, 30, 4, 42, 48, 43, 49, 31, 5, 67, 89, 119, 109, 126,
                                               100, 74, 80, 22, 12, 34, 56, 35, 57, 23, 13, 75, 81, 127, 101, 73, 83,
                                               125, 103, 33, 59, 21, 15, 20, 14, 32, 58, 124, 102, 72, 82, 16, 10, 36,
    ...
    public int computeCRC8(int data[], int start, int length)
    {
        int k = 0;
    ...
    public int computeCRC8(byte data[], int start, int length)
    {
        int k = 0;
        for (int l = 0; l < length; l++)
        {
    ...
    public int computeCRC16CCITT(byte data[], int start, int end)
    {
        System.out.println("data[]" + data.length + ", start=" + start + ", end=" + end);

        int k = 65535;
        for (int i1 = start; i1 < end; i1++)
        {
            int dataUnsigned = data[i1] < 0 ? data[i1] + 256 : data[i1];

            int l = dataUnsigned ^ k >> 8;
            k = (CRCUtils.CRC16_CCITT_LOOKUP_TABLE[l] ^ k << 8) & 0xffff;
        }
