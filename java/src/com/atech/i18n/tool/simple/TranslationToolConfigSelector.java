...
//		ATSwingUtils.getButton("Go !", 70, 80, 120, 25, panel, font, null, "go", this, da)
	...
	private boolean checkConfigFiles()
	{
		...
		//System.out.println("Config files found: " + this.configuration_files.size());
	...
	private Vector<ConfigurationEntry> discoverConfigFiles()
	{
		...
		for(int i=0; i<files.length; i++)
		{
			...
			ConfigurationEntry ce = this.getFileConfigurationEntry(files[i]);
...
/*	
	public String getFileDescription(File file)
	{
		...
		try
		{
			...
			PropertiesFile pf = new PropertiesFile(file.getParent() + file.getName());
